module Collaborations
  class Rater
    ...
    def find_highrated_items(user)
      @queries_aggregator.find_highrated_items(user).to_a
    end

    def find_lowrated_items(user)
      @queries_aggregator.find_lowrated_items(user).to_a
    end

    #TODO: unify interface

    def find_highrated_items_ids(user)
      @queries_aggregator.find_highrated_items_ids(user)
    end

    def find_lowrated_items_ids(user)
      @queries_aggregator.find_lowrated_items_ids(user)
    end
    ...
    def find_items(users)
      @queries_aggregator.find_items(users).to_a
    end
  end
end
